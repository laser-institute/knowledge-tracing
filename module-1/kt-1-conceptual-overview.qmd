...
title-slide-attributes: 
  data-notes: Hi, everyone, welcome to the lecture on Bayesian Knowledge Tracing, a method for knowledge inference.
---
...
-   Although its performance is as good as any of the modern alternatives, it still achieves good enough performance for many real-world tasks.
:::
...
-   The goal is not to measure overall skill for a broadly defined construct, such as arithmetic, but to measure a specific skill or knowledge component, such as adding two-digit numbers where no borrowing is needed
:::
...
-   it assumes in its classical form that each item must involve just a single latent skill. Each skill has four parameters. Only the first attempt on each item matters, or in other words, is included in the calculations. 
...
::: notes
The first parameter, PL0, is the probability that the student already knows the skill before their first opportunity to use it in problem-solving. In other words, students don’t come into our learning system as a blank state. They come in already having a certain probability of knowing what we want them to know.
:::
...
**P(L~n~)** Probability the student know it
...
**P(L~0~)** Probability the skill is already known before the first opportunity to use the skill
...
::: notes
If they get it wrong. The probability they knew it beforehand is the probability they knew it beforehand without this information times the probability of being a slip because they got it wrong, over that probability plus the probability that they didn't know it times the probability that they didn't guess it. So 0.6 times 0.8. The inverse of 0.4 is 0.6,  and the inverse of 0.2 is 0.8.
:::
...
2.   Baker, Corbin, and Alavin have proposed that guess and slip each have to be less than 0.5.
...
-   pyBKT
    ...
    -   [https://github.com/CAHLR/pyBK](https://github.com/CAHLR/pyBKT)
...
::: notes
It fits using expectation maximization because it takes too long to fit using brute force. Brute Force works when we've got an N to the fourth problem, but it doesn't work so well for an N to the eighth problem.
:::
