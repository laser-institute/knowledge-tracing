---
title: "Module 4 tem Response Theory and ELO"
...
title-slide-attributes: 
  data-notes: Today we are going to learn about knowledge inference methods with Item Response Theory.
---
...
A classic approach for assessment has been used for decades in tests and some online learning environments. In its classical form, it has some key limitations that make it less useful for assessment in online learning, but variants like ELO, and T-IRT (temporal IRT) address some of those limitations.
:::
...
-   so instead of saying, can Bob do 2 plus 5,  it says, how smart is Bob? Instead of asking if Bob knows this specific skill about snorkeling, it asks how much Bob knows about snorkeling in general
    ...
    -   check out more info about this example in SnorkeITutor 
:::
...
A typical use of IRT is:

-   to assess a student's current knowledge of a general topic X,  general domain, based on a sequence of items that are dichotomously scored
...
-   the first, and kind of the most controversial, is that there's only one latent trait or skill being measured per set of items

    -   this assumption is relaxed in the extension Cognitive Diagnosis Models (CDM) (Henson, Templin, & Willse, 2009), but in core IRT, there's just one skill, one general ability. How smart are you in math? 

-   another key assumption that's particularly questionable for online learning is that - no learning is occurring in between items 

    -   in other words, you have a testing situation with no help or feedback. Now, in most systems, where the student is actually getting better, we don't necessarily want to make this assumption
:::
...
-   from these parameters, we can compute the probability P of theta that the learner is going to get the current item correct
:::
...
Now the assumption is, again, that all items tap the same latent construct. Everything involves math, but they have different difficulties. This is a very different assumption than seen in PFA or BKT.
:::
...
Let's start with the Rasch (1PL) model. It's the simplest IRT model. Rasch is mathematically the same model with a different coefficient as another model called 1PL, but there are some different practices surrounding math.
:::
...
It's got no discriminability parameters. But it does have two sets of parameters, a set of parameters for student ability, and a set of parameters for item difficulty.
:::
...
Each learner has the ability theta, each item has difficulty b, and you can compute the probability that the student will get something right by a function involving an exponential function with theta minus b.

So we take the ability, we subtract the difficulty, they're on the same scale.
:::
...
-   when theta equals zero, for this graph, probably if correctness equals 0.5. This actually has to mean that b is equal to zero because for P of correct to be 0.5, theta and b have to be exactly balanced
:::
...
So in other words, if you look across the x-axis and at the thetas, theta zero is 50%, so this has to be b equals to zero as well. And the y-axis has the probability of correctness.
:::
...
1.  a green student who really knows their stuff,  theta of 3, probably getting it right at 95%

2.  a student who doesn't really know their stuff, theta of 0, probably of getting it right at 50% 

3.  a student who really doesn't know their stuff, theta of negative 3, probably of correctness of 5%
:::
...
-   the blue line, A equals 0.5,  has lower discriminability. It makes a much slower transition as students get better between correctness and incorrectness. Generally, you want items with relatively high discriminability, although in some cases being too high for discriminability isn't very useful because it only distinguishes a very small set of the range
:::
...
-   if you take a look at red, the line where a equals 0, there's no discriminability. People do equally well no matter what it is. I guess that would be a coin flip. How I already guess a coin flip
...
-   even though this is very discriminable, it may not be useful because it's only going to tell you the difference between, in this case, students with a theta of about negative 0.1, and students with a theta of positive 0.1. Anything below or above that range is just going to be completely certain to get it right or wrong
:::
...
Model degeneracy, which can't happen in Rasch but can happen in  2PL when you get a - discriminability below 0. In this case, the model tells you that the smarter you are, the more likely you are to get things wrong. And the less strong you are as a student, the more likely you are to get things right. This is even weirder than it is in BKT.
:::
...
The 3PL model, which is a more complex model, which adds a guessing parameter, c.
:::
...
-   the problem you get things right is the problem you guessed it plus the problem you didn't guess times the problem you knew it 
...
-   you can do this with expectation maximization as we discussed previously

-   you estimate knowledge and difficulty together. Then once you've done this, given an item difficulty estimate, you can take a new student and assess their knowledge in real-time
:::
...
t's worth briefly mentioning one extension to IRT which is ELO.

-   ELO has been around for a long time. It was used to rank chess players. It's a variant of a Rasch model that can be used in a running system,  which is a big difference than classical IRT, which makes estimates kind of post hoc. It continually estimates both item difficulty and the student's ability, updating both every time a student encounters an item
:::
...
An important extensionto to ELO is:

-   multivariate ELO, which allows an item to involve multiple skills and averages difficulty across skills
:::
...
-   MV-Glicko, which allows an item to involve multiple skills, averages difficulty across skills, and takes time between practices of skill into account, much like LKT extensions to PFA can do
:::
